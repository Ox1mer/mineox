...
public:
    ...
    std::map<char32_t, Character>& getCharacters() {
        return _characters;
    }
    ...
    void RenderText(Shader &s, const std::string& text, float x, float y, float scale, glm::vec3 color, glm::mat4& fontProjection)
    {
        s.use();
    ...
    void loadCharacters(unsigned int fontSize) {
        ...
        for (char32_t c = 0x0410; c <= 0x044F; ++c) {
            loadGlyph(c);
        }
        loadGlyph(0x0401); // Ё
        loadGlyph(0x0451); // ё
